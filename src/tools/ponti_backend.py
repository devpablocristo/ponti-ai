...
class PontiBackendClient:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._tenant_id = str(getattr(settings, "ponti_backend_tenant_id", "") or "").strip()
        self._tenant_header = str(getattr(settings, "ponti_backend_tenant_header", "") or "X-Tenant-Id").strip() or "X-Tenant-Id"
    ...
    def _build_headers(self, *, user_id: str) -> dict[str, str]:
        ...
        if self._tenant_id:
            headers[self._tenant_header] = self._tenant_id
        return headers
    ...
    async def get_json(
        ...
        *,
        ...
        try:
            body = response.json()
        except (ValueError, json.JSONDecodeError):  # noqa: PERF203
            logger.warning("ponti_backend_non_json_response", url=url, status=response.status_code)
